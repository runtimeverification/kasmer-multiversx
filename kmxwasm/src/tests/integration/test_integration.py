...
TEST_DATA = ROOT / 'kmxwasm' / 'src' / 'tests' / 'data'
INPUT_FILES = TEST_DATA.glob('*.json')
...
@pytest.mark.parametrize('test_file', TEST_DATA, ids=[str(test_file.relative_to(TEST_DATA)) for test_file in TEST_DATA])
